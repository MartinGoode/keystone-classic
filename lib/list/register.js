...
function register () {
	...
	if (this.get('inherits')) {
		...
		//Use the collection name in legacy format if it exists
		if (this.options.legacyCollection) {
			this.model = keystone.mongoose.model(this.key, this.schema,this.options.legacyCollection);
		}
		else {
			this.model = keystone.mongoose.model(this.key, this.schema,this.key);
		}
	}
