...
var UpdateModel = new mongoose.Schema(
	...
	{
		collection: keystone.prefixModel("App_Update",'app_updates')
	}
);
// MGOODE - Use legacy schema collection name
mongoose.model("App_Update", UpdateModel, UpdateModel.collection);

// Apply method - loads the available updates and applies any that haven't been, in order
exports.apply = function(callback) {
	// MGOODE - Use legacy schema collection name
	var Update = mongoose.model("App_Update",null,"app_updates");
	var updateCount = 0;
	var deferCount = 0;
	var skipCount = 0;
